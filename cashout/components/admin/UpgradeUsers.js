...
function UpgradeUsersComp(ctx) {
  ...
  const submitHandler = ({ emailList, accountType }) => {
    const data = emailList.split(",").map((emailList) => emailList.trim());
  ...
  return (
    <div>
      ...
      >
        <h1 className="sm:text-3xl mb-2 font-sans text-2xl">Upgrade Users</h1>
        {errorMessage && (
        ...
        <input
          ...
          aria-invalid={errors.emailList ? "true" : "false"}
        />
        {errors.emailList && (
          <p
     ...
     duration-300 w-full max-w-[39ch] border border-sky-500 border-solid uppercase py-2 px-2 text-cyan-900"
      ...
      <hr />
      {user ? (
        <p className="text-center mt-5">{user}</p>
      ) : (
...
export default UpgradeUsersComp;
